...
| **Exchange Flow** `/exchange-flow` | `load_exchange` — `data_exchange.csv` + harga dari `data_mvrv.csv`; jumlah net flow 7/14/30 hari per hari kalender (hari kosong = 0) | Judul "Exchange Net Flow & Balance". Overlay, BTC Log kanan; saklar **1d \| 7d \| 14d \| 30d** (`unit_default` **30d**, user 25 Sep 2026): 1d batang, lainnya area teal (masuk) / rust (keluar), sumbu kiri; pane **Balance** (Exchange Balance navy) **nyala awal** | Susunan disamakan dengan ETF Flows (user 25 Sep 2026). `net_flow` + = masuk bursa; flow = selisih saldo persis sejak 2012 kecuali 7 hari Okt–Des 2025 (bagian 6). Sebelum 2012 dan hari inflow = outflow = 0 dikosongkan. Detail di docstring `load_exchange` |
...
- Sumbu: `axis` bawaan + `separate_axis` saat Separate pane (aturan user: metrik berskala sama ke kanan saat Separate pane; **pengecualian Fear & Greed: semua sumbu kiri**). Sisi sumbu BTC per halaman = `MetricFamily.btc_axis_default` (bawaan kanan); pilihan BTC di kotak Display juga berlaku saat Separate pane (pindah sisi sumbu pane harga). `metric_range` (sumbu tetap, angka bulat tanpa desimal, di luar rentang kosong). Angka sumbu disembunyikan (lebar tetap) kalau semua garis di sisi itu mati. Seri mati diparkir ke skala lain supaya format sumbu tidak kacau.
...
- **Favorit (user 25 Sep 2026):** bintang ☆/★ di sebelah judul halaman (`metric_page`, key tombol `<key>_fav_on/off` untuk warna CSS) → kelompok **★ FAVORITES** di atas menu sidebar (`st.page_link`, urutan = urutan menu, halaman tetap juga di kelompok aslinya). Disimpan di cookie browser `dash_fav` (family.key dipisah koma, 1 tahun): dibaca `st.context.cookies` saat sesi mulai ke `st.session_state["favorit"]` (`FAV`), ditulis balik oleh iframe tersembunyi di sidebar (`app.py`). Per browser/HP, tidak sinkron. **Bintang juga di kanan tiap menu sidebar dan tautan FAVORITES** (kuning selalu tampil untuk favorit, abu hanya saat disorot): disisipkan JavaScript dari iframe itu ke menu bawaan Streamlit (tautan dikenali dari akhir alamat; halaman bawaan = ""), klik = menekan tombol Streamlit tersembunyi `favbtn_<key>` → rerun, halaman tidak pindah. MutationObserver memasang ulang bintang kalau menu digambar ulang. Bintang judul 22 px. **Belum dicek di Streamlit Cloud.**
...
- **Exchange Flow — 7 hari flow ≠ selisih saldo (dicek 25 Sep):** 8 Okt, 23 Okt, 5 Nov, 22 Nov, 2 Des, 14 Des, 21 Des 2025; di semua hari itu saldo turun lebih banyak dari flow (total ±72,7 ribu BTC keluar saldo tanpa tercatat outflow; mis. 21 Des flow +11, saldo −17.706). Di luar itu flow = selisih saldo persis sejak 2012. Sudah dicek: angka sama sejak salinan git tertua (20 Mei 2026); changelog Glassnode 2025 tanpa perubahan label bursa BTC Okt–Des; DefiLlama per bursa (USD, tanpa Coinbase) tidak bisa memastikan; berita tidak menemukan apa-apa. Dugaan (belum pasti): dompet dikeluarkan dari daftar bursa. Cara pasti: tanya ChartInspect / bandingkan CryptoQuant-Glassnode (berbayar).
...
- **`st.context.cookies` hanya dibaca saat sesi dimulai** (reload), tidak berubah per rerun → nilai selama sesi hidup di session_state. Iframe penulis cookie ditaruh di **sidebar** supaya urutan elemen halaman utama (iframe chart) tidak bergeser. Margin −16 px wadah teks juga berlaku di sidebar (judul FAVORITES sempat ditimpa tautan). Aturan tombol global `div[data-testid="stButton"] button` menang atas selektor pendek → selektor bintang harus lebih spesifik. **Iframe penulis cookie dimuat ulang tiap daftar favorit berubah → fungsi klik dari iframe lama mati** (bintang malah membuka halaman): `onclick` dipasang ulang setiap skrip berjalan. Bintang menu bergantung struktur DOM menu Streamlit 1.63 (`stSidebarNavLink`, `stPageLink-NavLink`) — cek lagi kalau Streamlit dinaikkan.
