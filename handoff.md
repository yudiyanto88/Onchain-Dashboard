...
- Pane: `price` (BTC Separate pane) → `main` → `extra` (pane bawah; `extra_label`, `extra_default`; skala `extra_scale` bawaan linear, "Log" untuk rasio positif; sisi sumbu dari `Series.axis`, bawaan kiri — semua pane bawah kanan kecuali OI Change). Garis pembatas antar pane putih 22 % (CSS `#panes > div + div::before`). Seri `pane="price"` digambar di pane harga sebelum garis BTC (sisi dari `Series.axis`; sisi yang dipakai batang selalu linear), saat Overlay/Hidden pindah ke pane bawah sumbu kanan. Zoom dan lebar sumbu tersinkron; sumbu waktu hanya di pane terbawah; slider rentang (isi BTC Log) di bawah semua.
...
| `data_supply.csv` | lth/sth_supply_btc, pct_lth/sth_in_profit/loss, percent_btc_in_profit/loss | dipakai (Supply in Profit, LTH/STH Supply) |
