...
Diperbarui 22 Sep 2026 (versi ringkas; terakhir sesudah `9bdc468`). Versi lengkap sebelum diringkas — cerita pengerjaan tiap fitur, opsi yang ditolak, hasil ukur piksel — ada di `archive/handoff_riwayat_2026-09-17.md` (buka hanya kalau perlu detail sejarah; nomor bagian 3.x yang disebut di bawah merujuk ke file itu).
...
- **Live: 17 halaman, 7 kelompok, dua label** — **ON-CHAIN** (Valuation · Profitability · Holder Behavior · Exchange) dan **MARKET** (Derivatives · Sentiment · Macro); daftar halaman = tabel bagian 3. Semua halaman: slider rentang, nyaman di HP, kontrol diingat per halaman, tombol Style di chart.
- **Langkah pertama sesi baru:**
  1. Cek server `dashboard-v2-uji` (bagian 2), buka halaman, lihat keadaannya sebelum mengubah apa pun.
  2. Cek log GitHub Actions run pertama sesudah `1ad0aa6`: Pipeline 23 (Yahoo), 24 (Cboe), 25 (FRED) lolos, dan `data_tradfi.csv` / `data_vix.csv` / `data_yields.csv` ikut ter-update (commit bot). Belum pernah diuji di Actions.
...
|---|---|---|---|
| **MVRV** `/` (key `market_valuation`) | `load_mvrv` — `data_mvrv.csv`; Rolling Z-Score 1y/2y/4y dihitung | BTC Separate pane, metrik & harga Log; LTH MVRV sumbu kanan; **Median MVRV `#6fb6de`** sumbu kiri, nyala awal; ref Neutral 1.0; pane **Z-Score** (Hidden, kanan): Z-Score navy + Rolling hijau `#97c459` (2y/4y mati awal) | Z-Score full-history untuk aturan, rolling untuk mata (KB MVRV). Tanpa pita extreme. Median MVRV dari `data_median_mvrv.csv`; bukan bagian framework v2 |
| **Price Levels** `/price-levels` | `load_price_levels` — `data_price_level.csv` + AVIV Mean/Upper dari `data_aviv.csv` | Overlay, satu sumbu kanan, Auto. STH RP, Realized Price, LTH RP, True Market Mean hijau zamrud `#3fa96b`, Median RP biru langit `#6fb6de`, AVIV Mean `#7b65d2`, AVIV Upper `#a58df0`, CVDD `#a8963f`; mati awal: MVRV 0σ, 200 DMA, 50 WMA, 200 WMA (abu). Pane bawah **Price / CVDD** (Hidden, Log, olive); Price / RP navy mati awal | Price/CVDD dipakai K4 framework v2 — garis ambang tidak dipasang. AVIV dihitung ulang `btc_price/aviv_ratio × band` (kolom `price_at_aviv_*` salah basis), 84 hari pertama disembunyikan (`_aviv_awal`). Detail di docstring `load_price_levels` |
...
- **Pipeline 19 / `data_treasury_2y.csv` boleh dibuang (menunggu keputusan user):** = DGS2 bulanan, digantikan `data_yields.csv`. Tidak ada pemakai (dicek seluruh repo, termasuk pembaca `data_master_all_metrics.csv`), dan tidak ada sejarah hilang (keduanya mulai 1 Jun 1976). Kalau dibuang: hapus juga dari daftar master dan sesuaikan `data_dictionary.md` (izin user).
...
- **VIX (21 Sep):** tanpa 0/macet/celah > 5 hari; 19 lonjakan > 40 % semuanya kejadian pasar asli (maks 82,69, 16 Mar 2020). **Yields (22 Sep):** tanpa ≤ 0; lonjakan terbesar 2Y −0,57 (13 Mar 2023, SVB); FRED telat 1–2 hari bursa.
