"""
STH-SOPR Bollinger Band — sinyal koreksi selama Z3 (bear recovery 2019 & 2023)
(Sama persis dengan analyze_asopr_bb_z3.py, tapi metrik = STH-SOPR bukan aSOPR)

Z3 def: RP (Realized Price) <= Price <= AVIV Mean.
BB dihitung dari full historical STH-SOPR series (rolling MA + rolling std),
lalu signal dicek hanya saat berada di dalam window Z3.
2019: difilter ke leg awal recovery saja (2019-04-02 -> 2019-06-19).
2023-2024: semua window Z3 dipakai.

MA period: 14, 20, 30 hari. Std multiplier: 1.5, 2.0. (6 kombinasi)
Signal = STH-SOPR menyentuh/melewati upper band (>=). Konsekutif hari
dikumpulkan jadi satu signal (tanggal signal = hari pertama menyentuh upper band).
"""
...
df = pl[['date', 'btc_price', 'realized_price']].merge(
    ...
    mo[['date', 'sth_sopr']], on='date', how='inner'
)
df = df.rename(columns={'price_at_aviv_mean': 'aviv_mean'})
df = df.dropna(subset=['btc_price', 'realized_price', 'aviv_mean', 'sth_sopr']).sort_values('date').reset_index(drop=True)
...
for p in ma_periods:
    df[f'ma_{p}'] = df['sth_sopr'].rolling(p).mean()
    df[f'std_{p}'] = df['sth_sopr'].rolling(p).std()
...
print("\n" + "=" * 78)
print("SIGNAL DETECTION PER KOMBINASI (STH-SOPR touch/cross upper band, dalam Z3)")
print("=" * 78)
...
for ma_p in ma_periods:
    for mult in std_mults:
        ...
        for ep in episode_windows:
            ...
            touch = ep_df['sth_sopr'] >= ep_df[upper_col]
