...
# --- CSS UNTUK PRESISI UI, TYPOGRAPHY, DAN UKURAN KONTROL ---
st.markdown("""
<style>
/* 1. Mencegah KPI terpotong di bagian atas (Mengatur jarak atas blok container) */
.block-container { padding-top: 2.5rem; padding-bottom: 1.5rem; max-width: 100%; }
div[data-testid="stVerticalBlock"] { gap: 0.5rem !important; }

/* 2. Membesarkan ukuran teks Radio di Sidebar agar terlihat seperti Tab Utama */
...
/* 3. Menyeragamkan ukuran font pada Label Kontrol (baris atas: Timeframe, SMA) */
...
/* 4. Menyeragamkan ukuran font "Full Screen" dan opsi Range agar sama dengan "Daily" */
...
/* 5. Menciutkan tinggi kotak Selectbox (Daily, 0d) agar tidak boros space */
...
/* 6. Menyeragamkan teks tombol Selection Metric (Pills) */
...
/* 7. Custom Styling Sidebar Panel */
...
# ==============================================================================
with st.sidebar:
    st.markdown("<h1 style='text-align: center; color: #ffffff; font-size: 2.2rem; font-weight: 800;'>MoneyBag Journal</h1>", unsafe_allow_html=True)
...
if selected_menu == "Price Levels":
    if not df_price_raw.empty:
        ...
        # Ditambahkan padding & line-height agar tidak terpotong (Clipping fix)
        def render_kpi_p(title, value, is_btc=False):
            ...
            st.markdown(f"<div style='padding-top: 8px;'><span style='color:{tc}; font-size:0.95rem; font-weight:600; line-height: 1.2;'>{title}</span><br><span style='color:{c}; font-size:1.4rem; font-weight:700;'>${value:,.2f}</span>{d}</div>", unsafe_allow_html=True)

        col_title, k1, k2, k3, k4, k5 = st.columns([1.5, 1, 1, 1, 1, 1], vertical_alignment="center")
        
        with col_title:
            st.markdown("<div style='border-right: 2px solid #333; padding-right: 15px; padding-top: 5px;'><h2 style='color: #a855f7; margin: 0; font-weight: 700;'>On-Chain Price Levels</h2></div>", unsafe_allow_html=True)
...
# ------------------------------------------------------------------------------
    ...
    if not df_mom_raw.empty:
        ...
        def render_kpi_m(title, value, threshold=1.0, is_money=False):
            ...
            st.markdown(f"<div style='padding-top: 8px;'><span style='color:{color}; font-size:0.95rem; font-weight:600; line-height: 1.2;'>{title}</span><br><span style='color:{color}; font-size:1.4rem; font-weight:700;'>{val_str}</span></div>", unsafe_allow_html=True)

        col_title, k1, k2, k3, k4, k5 = st.columns([1.5, 1, 1, 1, 1, 1], vertical_alignment="center")
        
        with col_title:
            st.markdown("<div style='border-right: 2px solid #333; padding-right: 15px; padding-top: 5px;'><h2 style='color: #a855f7; margin: 0; font-weight: 700;'>Profit & Loss</h2></div>", unsafe_allow_html=True)
            
        with k1: st.markdown(f"<div style='padding-top: 8px;'><span style='color:#a3a8b8; font-size:0.95rem; font-weight:600; line-height: 1.2;'>Current BTC Price</span><br><span style='color:#ffffff; font-size:1.4rem; font-weight:700;'>${btc_m:,.2f}</span></div>", unsafe_allow_html=True)
