...
# Menggunakan selector yang lebih mendalam untuk menembus class Streamlit
st.markdown("""
<style>
/* Kondisi Tombol OFF (Teks Tercoret & Abu-abu) */
[data-testid="stPills"] button[aria-pressed="false"] p,
[data-testid="stSegmentedControl"] button[aria-pressed="false"] p {
    text-decoration: line-through !important;
    color: #666666 !important;
}

/* Kondisi Tombol ON (Background Gradasi Ungu & Teks Putih) */
...
if 'log_p' not in st.session_state: st.session_state.log_p = True # Default Log Scale on
...
with tab1:
    if not df_price_raw.empty:
        ...
        # PANEL KONTROL
        col_fs, col_log, col_tf, col_sma, col_sma_cst, col_space, col_radio, col_custom = st.columns([1.2, 1.2, 1.5, 1.5, 1, 0.2, 5, 1.2], vertical_alignment="bottom")
        with col_fs: focus_p = st.toggle("Full Screen", key="tg_p")
        with col_log: st.session_state.log_p = st.toggle("Log Scale", value=st.session_state.log_p, key="lg_p", help="Gunakan ini agar skala tidak menjadi negatif")
        ...
        # RENDER CHART (Penyesuaian Skala Log & Ketebalan Garis)
        # Mode 1 = Logarithmic (Mencegah Skala Negatif), Mode 0 = Normal
        scale_mode = 1 if st.session_state.log_p else 0 
        ...
        chart_p_opts = {
            ...
            "rightPriceScale": {"mode": scale_mode} # Terapkan Log/Normal Scale
        }
        
        # Ketebalan BTC diturunkan menjadi 2
        ...
        for m in active_metrics_p:
            ...
            if base_m in colors_p:
                ...
                if is_sma:
                    # SMA line: Tipis (1) dan Putus-putus (2)
                    series_p.append({"type": 'Line', "data": get_s(df_p, f"{m_name}_SMA"), "options": {"color": m_color, "lineWidth": 1, "lineStyle": 2, "title": f"{m_name} SMA"}})
                else:
                    # Raw line: Diturunkan jadi ketebalan (1)
                    series_p.append({"type": 'Line', "data": get_s(df_p, m_name), "options": {"color": m_color, "lineWidth": 1, "lineStyle": 0, "title": m_name}})
...
with tab2:
    if not df_mom_raw.empty:
        ...
        # RENDER CHART SOPR (DENGAN 3 SKALA BERBEDA)
        chart_opts_sopr = {
            "layout": {"textColor": '#d1d4dc', "background": {"type": 'solid', "color": '#131722'}}, 
            "grid": {"vertLines": {"color": "rgba(42,46,57,0.3)"}, "horzLines": {"color": "rgba(42,46,57,0.3)"}}, 
            "crosshair": {"mode": 0}, 
            "height": 850 if focus_ms else 650, 
            "rightPriceScale": {"visible": True}, # Untuk BTC Price (Kanan)
            "leftPriceScale": {"visible": True}   # Untuk aSOPR & STH SOPR (Kiri)
        }
        
        series_sopr = [{"type": 'Line', "data": get_s(df_ms, 'BTC Price'), "options": {"color": '#f7931a', "lineWidth": 2, "priceScaleId": 'right', "title": 'BTC Price'}}]
        
        # Garis netral ikut skala kiri (karena itu tempatnya aSOPR & STH SOPR)
        ...
        for m in sel_sopr:
            ...
            if base_m in colors_sopr:
                ...
                # LOGIKA 3 SKALA: 
                # LTH SOPR dilempar ke 'lth_scale' (Skala Mandiri Ketiga, Invisible Axis). 
                # aSOPR & STH SOPR masuk ke 'left' (Kiri, Visible Axis).
