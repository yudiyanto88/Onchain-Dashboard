...
# ==============================================================================
# 1. PAGE CONFIGURATION & CUSTOM CSS (SHINY TECH)
# ==============================================================================
st.set_page_config(
...
# Injeksi CSS Khusus untuk Tombol st.pills
st.markdown("""
<style>
/* Kondisi OFF: Background transparan, teks abu-abu, DIBERI CORETAN (line-through) */
...
/* Kondisi ON: Gradasi ungu shiny, border terang, teks putih, TANPA CORETAN */
div[data-testid="stPill"] button[data-selected="true"], 
div[data-testid="stPill"] button[aria-pressed="true"] {
    background: linear-gradient(135deg, rgba(88, 28, 135, 0.9), rgba(147, 51, 234, 0.7)) !important;
...
def apply_filters(df, res_state, smooth_state, custom_smooth, time_state, custom_days, metrics_to_smooth):
    ...
    if w > 1:
        for c in metrics_to_smooth:
            if c in dff.columns:
                # Kolom duplikat khusus untuk garis putus-putus
...
with tab1:
    if not df_price_raw.empty:
        ...
        with controls_p:
            ...
            # Menambahkan tombol dinamis untuk SMA jika aktif
...
with tab2:
    if not df_mom_raw.empty:
        ...
        with controls_m:
            ...
            # Metrik Pilihan Chart 1 (SOPR)
            opts_m1_base = ['🔵 aSOPR', '🔴 STH SOPR', '🟢 LTH SOPR']
            all_opts_m1 = opts_m1_base.copy()
            if w_m > 1: all_opts_m1.extend([f"{m} (SMA {w_m})" for m in opts_m1_base])
            
            try: sel_m1 = st.pills("M1", all_opts_m1, default=opts_m1_base, selection_mode="multi", label_visibility="collapsed")
            except: sel_m1 = st.multiselect("M1", all_opts_m1, default=opts_m1_base, label_visibility="collapsed")
            
            # Metrik Pilihan Chart 2 (P/L)
            opts_m2_base = ['⚪ Net Realized PL', '🟣 STH P/L Ratio', '🟤 LTH P/L Ratio']
            all_opts_m2 = opts_m2_base.copy()
            if w_m > 1: all_opts_m2.extend([f"{m} (SMA {w_m})" for m in opts_m2_base])

            try: sel_m2 = st.pills("M2", all_opts_m2, default=opts_m2_base, selection_mode="multi", label_visibility="collapsed")
            except: sel_m2 = st.multiselect("M2", all_opts_m2, default=opts_m2_base, label_visibility="collapsed")
        ...
        # Render 2 Chart Terpisah
        with chart_m:
            tinggi_chart_m = 850 if focus_m else 650

            # --- CHART 1: SOPR GROUP ---
            chart_m1_opts = {"layout": {"textColor": '#d1d4dc', "background": {"type": 'solid', "color": '#131722'}}, "grid": {"vertLines": {"color": "rgba(42,46,57,0.3)"}, "horzLines": {"color": "rgba(42,46,57,0.3)"}}, "crosshair": {"mode": 0}, "height": tinggi_chart_m, "rightPriceScale": {"visible": True}, "leftPriceScale": {"visible": True}}
            series_m1 = [{"type": 'Line', "data": get_s(df_m, 'BTC Price'), "options": {"color": '#f7931a', "lineWidth": 2, "priceScaleId": 'right', "title": 'BTC Price'}}]
            
            df_m['Neutral_Line'] = 1.0
            series_m1.append({"type": 'Line', "data": get_s(df_m, 'Neutral_Line'), "options": {"color": '#ffffff', "lineWidth": 1, "lineStyle": 2, "priceScaleId": 'left', "title": 'Neutral (1.0)'}})

            colors_m1 = {'🔵 aSOPR': ('#00e6e6', 'aSOPR'), '🔴 STH SOPR': ('#ff4d4d', 'STH SOPR'), '🟢 LTH SOPR': ('#00cc66', 'LTH SOPR')}
            
            for m in sel_m1:
                is_sma = "(SMA" in m
                base_m = m.split(" (SMA")[0]
                if base_m in colors_m1:
                    m_col = colors_m1[base_m][0]
                    m_name = colors_m1[base_m][1]
                    if is_sma: series_m1.append({"type": 'Line', "data": get_s(df_m, f"{m_name}_SMA"), "options": {"color": m_col, "lineWidth": 1, "lineStyle": 2, "priceScaleId": 'left', "title": f"{m_name} SMA"}})
                    else: series_m1.append({"type": 'Line', "data": get_s(df_m, m_name), "options": {"color": m_col, "lineWidth": 2, "priceScaleId": 'left', "title": m_name}})

            renderLightweightCharts([{"chart": chart_m1_opts, "series": series_m1}], 'chart_sopr')
            
            st.markdown("<br><br>", unsafe_allow_html=True) # Jarak antar chart

            # --- CHART 2: REALIZED P/L GROUP ---
            chart_m2_opts = {"layout": {"textColor": '#d1d4dc', "background": {"type": 'solid', "color": '#131722'}}, "grid": {"vertLines": {"color": "rgba(42,46,57,0.3)"}, "horzLines": {"color": "rgba(42,46,57,0.3)"}}, "crosshair": {"mode": 0}, "height": tinggi_chart_m, "rightPriceScale": {"visible": True}, "leftPriceScale": {"visible": True}}
            series_m2 = [{"type": 'Line', "data": get_s(df_m, 'BTC Price'), "options": {"color": '#f7931a', "lineWidth": 2, "priceScaleId": 'right', "title": 'BTC Price'}}]

            colors_m2 = {'🟣 STH P/L Ratio': ('#cc33ff', 'STH P/L Ratio'), '🟤 LTH P/L Ratio': ('#cc9966', 'LTH P/L Ratio')}
            
            for m in sel_m2:
                is_sma = "(SMA" in m
                base_m = m.split(" (SMA")[0]
                if base_m in colors_m2:
                    m_col = colors_m2[base_m][0]
                    m_name = colors_m2[base_m][1]
                    if is_sma: series_m2.append({"type": 'Line', "data": get_s(df_m, f"{m_name}_SMA"), "options": {"color": m_col, "lineWidth": 1, "lineStyle": 2, "priceScaleId": 'left', "title": f"{m_name} SMA"}})
                    else: series_m2.append({"type": 'Line', "data": get_s(df_m, m_name), "options": {"color": m_col, "lineWidth": 2, "priceScaleId": 'left', "title": m_name}})
                
                elif base_m == '⚪ Net Realized PL':
                    if is_sma:
                        series_m2.append({"type": 'Line', "data": get_s(df_m, 'Net Realized PL_SMA'), "options": {"color": '#ffffff', "lineWidth": 1, "lineStyle": 2, "priceScaleId": 'right', "title": f"Net PL SMA"}})
                    else:
                        net_pl_raw = get_s(df_m, 'Net Realized PL')
                        for d in net_pl_raw: d['color'] = '#00cc66' if d['value'] >= 0 else '#ff4d4d'
                        series_m2.append({"type": 'Histogram', "data": net_pl_raw, "options": {"priceScaleId": 'right', "title": 'Net PL Raw'}})

            renderLightweightCharts([{"chart": chart_m2_opts, "series": series_m2}], 'chart_netpl')
