...
# --- CSS SUPER CUSTOM UNTUK SIDEBAR, CENTERING SELECTBOX & SPASI ---
st.markdown("""
<style>
/* A. STYLING SIDEBAR TABS */
...
/* B. FONT SIZE UNTUK SEMUA PANEL KONTROL */
...
div[data-testid="stSelectbox"] label, div[data-testid="stNumberInput"] label {
    padding-bottom: 2px !important; min-height: 0px !important;
}
div[data-testid="stToggle"] label p {
    margin-top: 2px !important; 
}

/* C. KOREKSI VERTIKAL (CENTERING) PADA SELECTBOX BACKGROUND */
...
# Inisialisasi Session State (Termasuk untuk Derivatives 'dr')
for key in ['tr_p', 'tr_ms', 'tr_mpl', 'tr_dr']:
    if key not in st.session_state: st.session_state[key] = "All Time"
for key in ['cd_p', 'cd_ms', 'cd_mpl', 'cd_dr']:
    if key not in st.session_state: st.session_state[key] = 120
for key in ['tf_p', 'tf_ms', 'tf_mpl', 'tf_dr']:
    if key not in st.session_state: st.session_state[key] = "Daily"
for key in ['sma_p', 'sma_ms', 'sma_mpl', 'sma_dr']:
    if key not in st.session_state: st.session_state[key] = "0d"
for key in ['cs_p', 'cs_ms', 'cs_mpl', 'cs_dr']:
...
def load_data_derivatives():
    try:
        ...
        df.rename(columns={'date': 'Date', 'btc_price': 'BTC Price', 'total_oi': 'Total Open Interest', 'funding_rate': 'Funding Rate'}, inplace=True)
        df['Date'] = pd.to_datetime(df['Date'], errors='coerce')
        return df.dropna(subset=['Date']).sort_values('Date')
    except: return pd.DataFrame()
...
# Format HTML Konsisten untuk Header (Memastikan tinggi border selalu 50px)
TITLE_STYLE = "<div style='border-right: 2px solid #333; height: 50px; padding-right: 15px; display: flex; flex-direction: column; justify-content: center;'><h3 style='color: #a855f7; margin: 0; font-weight: 700; font-size: 1.4rem; line-height: 1.2;'>{}<br><span style='font-size: 1rem; color: {};'>{}</span></h3></div>"
KPI_STYLE = "<div style='height: 50px; display: flex; flex-direction: column; justify-content: center;'><span style='color:{}; font-size:0.95rem; font-weight:600; line-height: 1.1;'>{}</span><span style='color:{}; font-size:1.4rem; font-weight:700; line-height: 1.3;'>{}</span></div>"

def render_kpi(title, value, threshold=None, is_money=False):
    if pd.isna(value) or value == 0: color = "#a3a8b8"
    else: 
        if threshold is None: color = "#ffffff"
        else: color = "#00cc66" if value >= threshold else "#ff4d4d"
    
    val_str = f"${value:,.2f}" if is_money else (f"{value:.4f}" if threshold is not None else str(value))
    st.markdown(KPI_STYLE.format(color if threshold is None else "#a3a8b8", title, color, val_str), unsafe_allow_html=True)
...
if selected_menu == "Price Levels":
    if not df_price_raw.empty:
        ...
        with col_title:
            st.markdown(TITLE_STYLE.format("On-Chain Price Levels", "transparent", "."), unsafe_allow_html=True)
            
        with k1: render_kpi("Current BTC Price", btc_p, is_money=True)
        with k2: render_kpi("STH Cost Basis", last_p.get('STH Cost Basis', 0), is_money=True)
        with k3: render_kpi("LTH Cost Basis", last_p.get('LTH Cost Basis', 0), is_money=True)
        with k4: render_kpi("Realized Price", last_p.get('Realized Price', 0), is_money=True)
        with k5: render_kpi("True Market Mean", last_p.get('True Market Mean', 0), is_money=True)
    ...
    if not df_mom_raw.empty:
        ...
        with col_title_1:
            st.markdown(TITLE_STYLE.format("Profit & Loss", "#d1d4dc", "SOPR Metric"), unsafe_allow_html=True)
            
        with k1_1: render_kpi("Current BTC Price", btc_p, is_money=True)
        with k2_1: render_kpi("aSOPR", last_m.get('aSOPR', 0), 1.0)
        with k3_1: render_kpi("LTH SOPR", last_m.get('LTH SOPR', 0), 1.0)
        with k4_1: render_kpi("STH SOPR", last_m.get('STH SOPR', 0), 1.0)
        ...
        # Injeksi Skala ratio untuk SOPR
        chart_opts_sopr = {"layout": {"textColor": '#d1d4dc', "background": {"type": 'solid', "color": '#131722'}}, "grid": {"vertLines": {"color": "rgba(42,46,57,0.3)"}, "horzLines": {"color": "rgba(42,46,57,0.3)"}}, "crosshair": {"mode": 0}, "height": 850 if focus_ms else 650, "rightPriceScale": {"visible": True}, "leftPriceScale": {"visible": True}, "ratio": {"visible": True, "position": "left", "autoScale": True}}
        ...
        for m in sel_sopr:
            ...
            if base_m in colors_sopr:
                ...
                target_scale = "ratio" if base_m == '🟢 LTH SOPR' else "left"
        ...
        with col_title_2:
            st.markdown(TITLE_STYLE.format("Profit & Loss", "#d1d4dc", "Realized P&L Metric"), unsafe_allow_html=True)
            
        with k1_2: render_kpi("Current BTC Price", btc_p, is_money=True)
        with k2_2: render_kpi("Net Realized PL", last_m.get('Net Realized PL', 0), 0.0, is_money=True)
        with k3_2: render_kpi("STH P/L Ratio", last_m.get('STH P/L Ratio', 0), 1.0)
        with k4_2: render_kpi("LTH P/L Ratio", last_m.get('LTH P/L Ratio', 0), 1.0)
        ...
        # 3 SKALA UNTUK P&L: BTC Kanan (right), NetPL Kiri (left), Ratio Kiri Custom (ratio)
        chart_opts_pl = {"layout": {"textColor": '#d1d4dc', "background": {"type": 'solid', "color": '#131722'}}, "grid": {"vertLines": {"color": "rgba(42,46,57,0.3)"}, "horzLines": {"color": "rgba(42,46,57,0.3)"}}, "crosshair": {"mode": 0}, "height": 850 if focus_mpl else 650, "rightPriceScale": {"visible": True}, "leftPriceScale": {"visible": True}, "ratio": {"visible": True, "position": "left", "autoScale": True}}
        ...
        for m in sel_pl:
            ...
            if base_m in colors_pl:
                ...
                if is_sma: series_pl.append({"type": 'Line', "data": get_s(df_mpl, f"{c_name}_SMA"), "options": {"color": c_col, "lineWidth": 1, "lineStyle": 2, "priceScaleId": 'ratio', "title": f"{c_name} SMA"}})
                else: series_pl.append({"type": 'Line', "data": get_s(df_mpl, c_name), "options": {"color": c_col, "lineWidth": 1, "priceScaleId": 'ratio', "title": c_name}})
            elif base_m == '⚪ Net Realized PL':
                if is_sma: series_pl.append({"type": 'Line', "data": get_s(df_mpl, 'Net Realized PL_SMA'), "options": {"color": '#ffffff', "lineWidth": 1, "lineStyle": 2, "priceScaleId": 'left', "title": "Net PL SMA"}})
                else:
...
# ------------------------------------------------------------------------------
    ...
    if not df_deriv_raw.empty:
        df_dr, w_dr = apply_filters(df_deriv_raw, st.session_state.tf_dr, st.session_state.sma_dr, st.session_state.cs_dr, st.session_state.tr_dr, st.session_state.cd_dr, ['Total Open Interest', 'Funding Rate'])
        
        last_dr = df_dr.iloc[-1]
        btc_dr = last_dr.get('BTC Price', 0)
        oi_val = last_dr.get('Total Open Interest', 0)
        fr_val = last_dr.get('Funding Rate', 0)

        col_title, k1, k2, k3, k4, k5 = st.columns([1.5, 1, 1, 1, 1, 1], vertical_alignment="center")
        with col_title:
            st.markdown(TITLE_STYLE.format("Derivatives", "#d1d4dc", "OI & Funding Rate"), unsafe_allow_html=True)
            
        with k1: render_kpi("Current BTC Price", btc_dr, is_money=True)
        with k2: render_kpi("Total Open Interest", oi_val, is_money=True)
        with k3: render_kpi("Funding Rate (%)", fr_val, 0.0)
        
        st.markdown("---")

        col_fs, col_tf, col_sma, col_sma_cst, col_radio, col_custom = st.columns([1, 1.2, 1.2, 1, 6, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs: focus_dr = st.toggle("Full Screen", key="tg_dr")
        with col_tf: st.session_state.tf_dr = st.selectbox("Timeframe", ["Daily", "3 Days", "Weekly", "Monthly"], index=["Daily", "3 Days", "Weekly", "Monthly"].index(st.session_state.tf_dr), key="tfs_dr")
        with col_sma: st.session_state.sma_dr = st.selectbox("SMA", ["0d", "7d", "14d", "30d", "Custom"], index=["0d", "7d", "14d", "30d", "Custom"].index(st.session_state.sma_dr), key="smas_dr")
        with col_sma_cst:
            if st.session_state.sma_dr == "Custom": st.session_state.cs_dr = st.number_input("Days", min_value=1, value=st.session_state.cs_dr, label_visibility="collapsed", key="cst_dr")
        with col_radio:
            c_idx_dr = t_opts.index(st.session_state.tr_dr) if st.session_state.tr_dr in t_opts else 5
            st.session_state.tr_dr = st.radio("Range:", t_opts, index=c_idx_dr, horizontal=True, label_visibility="collapsed", key="rg_dr")
        with col_custom:
            if st.session_state.tr_dr == "Custom": st.session_state.cd_dr = st.number_input("Days back", min_value=7, value=st.session_state.cd_dr, label_visibility="collapsed", key="cdin_dr")
            
        opts_dr_base = ['🔵 Open Interest', '🔴 Funding Rate']
        all_opts_dr = opts_dr_base.copy()
        if w_dr > 1: all_opts_dr.extend([f"{m} (SMA {w_dr})" for m in opts_dr_base])

        try: sel_dr = st.pills("Derivatives", all_opts_dr, default=opts_dr_base, selection_mode="multi", label_visibility="collapsed")
        except: sel_dr = st.multiselect("Derivatives", all_opts_dr, default=opts_dr_base, label_visibility="collapsed")

        # 3 SKALA UNTUK DERIVATIVES: BTC (right), OI (left), Funding (funding)
        chart_opts_dr = {"layout": {"textColor": '#d1d4dc', "background": {"type": 'solid', "color": '#131722'}}, "grid": {"vertLines": {"color": "rgba(42,46,57,0.3)"}, "horzLines": {"color": "rgba(42,46,57,0.3)"}}, "crosshair": {"mode": 0}, "height": 850 if focus_dr else 650, "rightPriceScale": {"visible": True}, "leftPriceScale": {"visible": True}, "funding": {"visible": True, "position": "left", "autoScale": True}}
        series_dr = [{"type": 'Line', "data": get_s(df_dr, 'BTC Price'), "options": {"color": '#f7931a', "lineWidth": 2, "priceScaleId": 'right', "title": 'BTC Price'}}]
        ...
        for m in sel_dr:
            ...
            if base_m == '🔵 Open Interest':
                if is_sma: series_dr.append({"type": 'Line', "data": get_s(df_dr, 'Total Open Interest_SMA'), "options": {"color": '#4da6ff', "lineWidth": 1, "lineStyle": 2, "priceScaleId": 'left', "title": "OI SMA"}})
                else: series_dr.append({"type": 'Area', "data": get_s(df_dr, 'Total Open Interest'), "options": {"lineColor": '#4da6ff', "topColor": 'rgba(77, 166, 255, 0.4)', "bottomColor": 'rgba(77, 166, 255, 0.05)', "lineWidth": 2, "priceScaleId": 'left', "title": 'Open Interest'}})
            
            elif base_m == '🔴 Funding Rate':
                if is_sma: series_dr.append({"type": 'Line', "data": get_s(df_dr, 'Funding Rate_SMA'), "options": {"color": '#ffffff', "lineWidth": 1, "lineStyle": 2, "priceScaleId": 'funding', "title": "Funding SMA"}})
                else:
                    fr_raw = get_s(df_dr, 'Funding Rate')
                    for d in fr_raw: d['color'] = '#00cc66' if d['value'] >= 0 else '#ff4d4d'
                    series_dr.append({"type": 'Histogram', "data": fr_raw, "options": {"priceScaleId": 'funding', "title": 'Funding Rate'}})

        renderLightweightCharts([{"chart": chart_opts_dr, "series": series_dr}], 'chart_derivatives')
    else:
        st.info("Data Derivatives kosong. Pastikan GitHub Actions telah menarik data baru.")
