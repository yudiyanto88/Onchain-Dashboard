...
# ==============================================================================
# 1. PAGE CONFIGURATION, SESSION STATE & CSS COMPACT
# ==============================================================================
st.set_page_config(
...
# --- CSS UNTUK MERAPATKAN JARAK ANTAR ELEMEN (1 FRAME DISPLAY) ---
st.markdown("""
<style>
...
h3, h4 { margin: 0 !important; padding-top: 0.5rem !important; padding-bottom: 0 !important; }
/* Mengecilkan sedikit ukuran font kpi agar tidak memakan ruang vertical */
.kpi-title { font-size: 0.85rem; color: #a3a8b8; font-weight: 600; }
.kpi-value { font-size: 1.25rem; font-weight: 700; margin-top: -2px;}
</style>
""", unsafe_allow_html=True)

# Inisialisasi State Independen untuk Tab 1 (Price)
for key in ['tr_p', 'tf_p', 'sma_p']:
    if key not in st.session_state: st.session_state[key] = "All Time" if key == 'tr_p' else "Daily" if key == 'tf_p' else "0d"
for key in ['cd_p', 'cs_p']:
    if key not in st.session_state: st.session_state[key] = 120 if key == 'cd_p' else 50

# Inisialisasi State Independen untuk Tab 2 (SOPR Group)
for key in ['tr_ms', 'tf_ms', 'sma_ms']:
    if key not in st.session_state: st.session_state[key] = "All Time" if key == 'tr_ms' else "Daily" if key == 'tf_ms' else "0d"
for key in ['cd_ms', 'cs_ms']:
    if key not in st.session_state: st.session_state[key] = 120 if key == 'cd_ms' else 50

# Inisialisasi State Independen untuk Tab 2 (Realized P/L Group)
for key in ['tr_mpl', 'tf_mpl', 'sma_mpl']:
    if key not in st.session_state: st.session_state[key] = "All Time" if key == 'tr_mpl' else "Daily" if key == 'tf_mpl' else "0d"
for key in ['cd_mpl', 'cs_mpl']:
    if key not in st.session_state: st.session_state[key] = 120 if key == 'cd_mpl' else 50
...
with tab1:
    if not df_price_raw.empty:
        ...
        # 1. JUDUL & KPI COMPACT
        st.markdown("<h3>On-Chain Price Levels</h3>", unsafe_allow_html=True)
        ...
        def render_kpi_p(title, value, is_btc=False):
            if is_btc or pd.isna(value) or value == 0:
                ...
                tc, ar = c, "↑" if ip else "↓"
                d = f"<div style='margin-top:2px;'><span style='color:{c}; font-size:0.75rem; background-color:{c}20; padding:1px 4px; border-radius:3px;'>{ar} {abs(dp):.2f}%</span></div>"
            st.markdown(f"<div><div class='kpi-title' style='color:{tc};'>{title}</div><div class='kpi-value' style='color:{c};'>${value:,.2f}</div>{d}</div>", unsafe_allow_html=True)
...
with tab2:
    if not df_mom_raw.empty:
        ...
        def render_kpi_m(title, value, threshold=1.0, is_money=False):
            ...
            st.markdown(f"<div><div class='kpi-title' style='color:{color};'>{title}</div><div class='kpi-value' style='color:{color};'>{val_str}</div></div>", unsafe_allow_html=True)

        k1, k2, k3, k4, k5 = st.columns(5)
        with k1: st.markdown(f"<div><div class='kpi-title'>Current BTC Price</div><div class='kpi-value' style='color:#ffffff;'>${btc_m:,.2f}</div></div>", unsafe_allow_html=True)
        ...
        # ===========================================
        # CHART 1: SOPR GROUP (DENGAN 3 SKALA)
        # ===========================================
        st.markdown("<h4>SOPR Oscillators</h4>", unsafe_allow_html=True)
        ...
        # MENAMBAHKAN SKALA KETIGA (lth_scale) UNTUK LTH SOPR
        chart_opts_sopr = {
            ...
            "lth_scale": {"visible": True, "position": "left"} # Skala khusus yang muncul terpisah di kiri
        }
        
        # BTC DI SKALA KANAN ('right')
        ...
        # NEUTRAL LINE & STH/aSOPR DI SKALA KIRI ('left')
        ...
        for m in sel_sopr:
            ...
            if base_m in colors_sopr:
                ...
                # LOGIKA SKALA: Jika LTH SOPR, masukkan ke skala ketiga ('lth_scale'). Sisanya ke kiri ('left')
                ...
                if is_sma: 
                    ...
                    series_sopr.append({"type": 'Line', "data": get_s(df_ms, c_name), "options": {"color": c_col, "lineWidth": 1, "priceScaleId": target_scale, "title": c_name}})
        ...
        # ===========================================
        st.markdown("<h4>Realized Profit & Loss</h4>", unsafe_allow_html=True)
