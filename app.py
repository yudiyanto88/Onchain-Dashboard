...
with tab1:
    if not df_price_raw.empty:
        header_container_p = st.container()
        controls_container_p = st.container()
        chart_container_p = st.container()

        # --- CONTROLS ---
        with controls_container_p:
            ...
            active_metrics_p = st.multiselect("Metrics", metric_opts_p, default=metric_opts_p, label_visibility="collapsed", key="ms_p")
        ...
        with header_container_p:
            if not focus_mode_p:
                ...
                def render_kpi(title, value, is_btc=False):
                    ...
                    st.markdown(f"<div style='padding-bottom:10px;'><span style='color:{tc}; font-size:0.95rem; font-weight:600; margin-bottom:2px;'>{title}</span><br><span style='color:{c}; font-size:1.4rem; font-weight:700;'>${value:,.2f}</span>{d}</div>", unsafe_allow_html=True)
...
with tab2:
    if not df_mom_raw.empty:
        header_container_m = st.container()
        controls_container_m = st.container()
        chart_container_m = st.container()

        # --- CONTROLS ---
        with controls_container_m:
            col_fs, col_tf, col_sma, col_sma_cst, col_space, col_radio, col_custom = st.columns([1.2, 1.5, 1.5, 1, 0.5, 5, 1.2], vertical_alignment="bottom")
            ...
            with col_radio: st.radio("Range:", t_opts, index=c_idx, horizontal=True, label_visibility="collapsed", key="rg_m_d", disabled=True)
            
            # Metric Selection (Side by side tanpa judul grup)
            col_sel1, col_sel2 = st.columns(2)
            m_sopr = ['🟠 aSOPR', '🔴 STH SOPR', '🔵 LTH SOPR']
            m_pl = ['🟢 Net Realized PL', '⚪ STH P/L Ratio', '🟣 LTH P/L Ratio']
            with col_sel1: sel_sopr = st.multiselect("m1", m_sopr, default=['🟠 aSOPR'], label_visibility="collapsed", key="ms_m1")
            with col_sel2: sel_pl = st.multiselect("m2", m_pl, default=['🟢 Net Realized PL'], label_visibility="collapsed", key="ms_m2")
            st.markdown("<br>", unsafe_allow_html=True)
        ...
        with header_container_m:
            if not focus_mode_m:
                ...
                # Fungsi khusus KPI Tab 2 (Warna Hijau/Merah murni dari batas logika tanpa persentase)
                def render_kpi_tab2(title, value, is_btc=False, is_usd=False):
                    if is_btc or pd.isna(value) or value == 0:
                        c, tc = "#ffffff", "#a3a8b8"
                    ...
                    <div style="display: flex; flex-direction: column; padding-bottom: 10px;">
                        <span style="color: {tc}; font-size: 0.95rem; font-weight: 600; margin-bottom: 2px;">{title}</span>
                        <span style="color: {c}; font-size: 1.4rem; font-weight: 700;">{val_str}</span>
                    </div>
                    """, unsafe_allow_html=True)

                k1, k2, k3, k4, k5 = st.columns(5)
                with k1: render_kpi_tab2("Current BTC Price", btc_m, is_btc=True)
                with k2: render_kpi_tab2("aSOPR", last_m.get('aSOPR', 0))
                with k3: render_kpi_tab2("STH SOPR", last_m.get('STH SOPR', 0))
                with k4: render_kpi_tab2("LTH SOPR", last_m.get('LTH SOPR', 0))
                with k5: render_kpi_tab2("Net Realized PL", last_m.get('Net Realized PL', 0), is_usd=True)
                st.markdown("---")

        # --- CHART ---
        with chart_container_m:
            chart_m_opts = {
            ...
            colors_m = {'🟠 aSOPR': ('#ff9933', 'aSOPR'), '🔴 STH SOPR': ('#ff4d4d', 'STH SOPR'), '🔵 LTH SOPR': ('#4da6ff', 'LTH SOPR'), '⚪ STH P/L Ratio': ('#ffffff', 'STH P/L Ratio'), '🟣 LTH P/L Ratio': ('#cc33ff', 'LTH P/L Ratio')}
            
            for m in sel_sopr + sel_pl:
                if m in colors_m: series_m.append({"type": 'Line', "data": get_s(df_m, colors_m[m][1]), "options": {"color": colors_m[m][0], "lineWidth": 2, "priceScaleId": 'left', "title": colors_m[m][1]}})
            
            if '🟢 Net Realized PL' in sel_pl:
