...
div[data-testid="stButton"] button {
    ...
    font-size: 0.85rem !important; margin-top: 4px !important;
}
...
if selected_menu == "Price Levels":
    if not df_price_raw.empty:
        ...
        col_fs, col_rz, col_tf, col_sma, col_sma_cst, col_radio, col_custom = st.columns([1, 1, 1.2, 1.2, 1, 5, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs: focus_p = st.toggle("Full Screen", key="tg_p")
        with col_rz: st.button("🔄 Reset", key="rz_p")
        ...
        # SATU SKALA: BTC dan Metrik On-Chain semuanya di Kanan
        chart_p_opts = {"layout": {"textColor": '#d1d4dc', "background": {"type": 'solid', "color": '#131722'}}, "grid": {"vertLines": {"color": "rgba(42,46,57,0.3)"}, "horzLines": {"color": "rgba(42,46,57,0.3)"}}, "crosshair": {"mode": 0}, "height": 850 if focus_p else 650, "rightPriceScale": {"visible": True}}
        ...
        for m in active_metrics_p:
            ...
            if base_m in colors_p:
                c_col, c_name = colors_p[base_m]
                if is_sma: series_p.append({"type": 'Line', "data": get_s(df_p, f"{c_name}_SMA"), "options": {"color": c_col, "lineWidth": 1, "lineStyle": 2, "priceScaleId": 'right', "title": f"{c_name} SMA"}})
                else: series_p.append({"type": 'Line', "data": get_s(df_p, c_name), "options": {"color": c_col, "lineWidth": 1, "priceScaleId": 'right', "title": c_name}})
...
# ------------------------------------------------------------------------------
    ...
    if not df_mvrv_raw.empty:
        ...
        col_fs_mv, col_rz_mv, col_tf_mv, col_sma_mv, col_sma_cst_mv, col_radio_mv, col_custom_mv = st.columns([1, 1, 1.2, 1.2, 1, 5, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs_mv: focus_mv = st.toggle("Full Screen", key="tg_mv")
        with col_rz_mv: st.button("🔄 Reset", key="rz_mv")
...
# ------------------------------------------------------------------------------
    ...
    if not df_mom_raw.empty:
        ...
        col_fs_ms, col_rz_ms, col_tf_ms, col_sma_ms, col_sma_cst_ms, col_radio_ms, col_custom_ms = st.columns([1, 1, 1.2, 1.2, 1, 5, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs_ms: focus_ms = st.toggle("Full Screen", key="tg_ms")
        with col_rz_ms: st.button("🔄 Reset", key="rz_ms")
        ...
        col_fs_mpl, col_rz_mpl, col_tf_mpl, col_sma_mpl, col_sma_cst_mpl, col_radio_mpl, col_custom_mpl = st.columns([1, 1, 1.2, 1.2, 1, 5, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs_mpl: focus_mpl = st.toggle("Full Screen", key="tg_mpl")
        with col_rz_mpl: st.button("🔄 Reset", key="rz_mpl")
        ...
        chart_opts_pl = {
            ...
            "rightPriceScale": {"visible": True}, "leftPriceScale": {"visible": True}, "ratio_scale": {"visible": True, "position": "left", "autoScale": True}  
        }
        ...
        col_fs_nupl, col_rz_nupl, col_tf_nupl, col_sma_nupl, col_sma_cst_nupl, col_radio_nupl, col_custom_nupl = st.columns([1, 1, 1.2, 1.2, 1, 5, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs_nupl: focus_nupl = st.toggle("Full Screen", key="tg_nupl")
        with col_rz_nupl: st.button("🔄 Reset", key="rz_nupl")
...
# ------------------------------------------------------------------------------
    ...
    if not df_supply_raw.empty:
        ...
        col_fs_sd, col_rz_sd, col_tf_sd, col_sma_sd, col_sma_cst_sd, col_radio_sd, col_custom_sd = st.columns([1, 1, 1.2, 1.2, 1, 5, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs_sd: focus_sd = st.toggle("Full Screen", key="tg_sd")
        with col_rz_sd: st.button("🔄 Reset", key="rz_sd")
        ...
        col_fs_sd2, col_rz_sd2, col_tf_sd2, col_sma_sd2, col_sma_cst_sd2, col_radio_sd2, col_custom_sd2 = st.columns([1, 1, 1.2, 1.2, 1, 5, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs_sd2: focus_sd2 = st.toggle("Full Screen", key="tg_sd2")
        with col_rz_sd2: st.button("🔄 Reset", key="rz_sd2")
...
# ------------------------------------------------------------------------------
    ...
    if not df_deriv_raw.empty:
        ...
        col_fs_d, col_rz_d, col_tf_d, col_sma_d, col_sma_cst_d, col_radio_d, col_custom_d = st.columns([1, 1, 1.2, 1.2, 1, 5, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs_d: focus_d = st.toggle("Full Screen", key="tg_d")
        with col_rz_d: st.button("🔄 Reset", key="rz_d")
...
# ------------------------------------------------------------------------------
    ...
    if not df_sentiment_raw.empty:
        ...
        col_fs_gt, col_rz_gt, col_tf_gt, col_sma_gt, col_mode_gt, col_radio_gt, col_custom_gt = st.columns([1, 1, 1.2, 1.2, 1.2, 5.5, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs_gt: focus_gt = st.toggle("Full Screen", key="tg_gt")
        with col_rz_gt: st.button("🔄 Reset", key="rz_gt")
        ...
        col_fs_wk, col_rz_wk, col_tf_wk, col_sma_wk, col_mode_wk, col_radio_wk, col_custom_wk = st.columns([1, 1, 1.2, 1.2, 1.2, 5.5, 1.2], vertical_alignment="bottom", gap="small")
        with col_fs_wk: focus_wk = st.toggle("Full Screen", key="tg_wk")
        with col_rz_wk: st.button("🔄 Reset", key="rz_wk")
        ...
        # CHART 3: FEAR & GREED INDEX (INJEKSI MARKERS & GARIS PUTIH TANPA HISTOGRAM)
        if not df_fg_raw.empty:
            ...
            col_fs_fg, col_rz_fg, col_empty1, col_empty2, col_radio_fg, col_custom_fg = st.columns([1, 1, 1.2, 1.2, 5.5, 1.2], vertical_alignment="bottom", gap="small")
            with col_fs_fg: focus_fg = st.toggle("Full Screen", key="tg_fg")
            with col_rz_fg: st.button("🔄 Reset", key="rz_fg")
            with col_radio_fg:
            ...
            # Skala Kiri dimatikan secara visual
            ...
            for m in sel_fg:
                ...
                # Render F&G Value sebagai garis tak kasat mata agar angkanya tetap muncul di legenda saat di hover (Crosshair)
                series_fg.append({"type": 'Line', "data": fg_raw, "options": {"color": 'rgba(0,0,0,0)', "lineWidth": 0, "crosshairMarkerVisible": False, "priceScaleId": 'left', "title": 'F&G Index'}})
